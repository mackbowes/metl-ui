...
export default function MinterTable(props) {
  let data = props.data;
  if (data.length < 10) {
    for (let i = 0; i < 10; i++) {
          ...
          <Box>
            {data.map((entry, index) => {
              ...
              return (
                ...
                >
                  <Box w="140px" p="4px">
                    <Text>{entry.isMinter === true && entry.address}</Text>
                  </Box>
                  <Box w="145px" p="4px">
                    <Text>{entry.isMinter === true && entry.name}</Text>
                  </Box>
                  <Box w="145px" p="4px">
                    <Text>
                      {entry.isMinter !== "" &&
                      typeof entry.isMinter === "boolean" ? (
                        entry.isMinter === true ? (
